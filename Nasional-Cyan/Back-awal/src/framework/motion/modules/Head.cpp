...
Head::Head()
{
	...
	m_BottomLimit = Kinematics::EYE_TILT_OFFSET_ANGLE - 55;
	
	m_Pan_Home = 0.0;
	m_Tilt_Home = Kinematics::EYE_TILT_OFFSET_ANGLE - 0.0;
...
bool Start=true;
int count=0;
double time_move=0.8;  //0.8 setting waktu putar kepala
int move_value=4;
int pan_move=0;
int tilt_move=0;
...
void Head::InitTracking(int start)
{
    ...
    */ 
	...
	else if(move==2)//right top
	{
		...
		if(tilt > (m_TopLimit - 3))tilt_move=0; else tilt_move=move_value;
		if(pan < (m_RightLimit + 3) && tilt > (m_TopLimit - 3)) move=5;
