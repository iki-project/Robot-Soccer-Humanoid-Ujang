...
void jalantempat2()
{
	...
	Walking::GetInstance()->P_OFFSET=-7.0;
	Walking::GetInstance()->HIP_PITCH_OFFSET = 27.0;
	Walking::GetInstance()->Z_MOVE_AMPLITUDE = 25.0;
...
void maju()
{
	...
	Walking::GetInstance()->P_OFFSET=-7.5;
	Walking::GetInstance()->HIP_PITCH_OFFSET = 29.0;
    Walking::GetInstance()->Z_MOVE_AMPLITUDE = 37.0;//foot heigh
	Walking::GetInstance()->X_MOVE_AMPLITUDE = 10;//maju mundur}
//	Walking::GetInstance()->A_MOVE_A = 0;
	fprintf(stderr, "maju\n");
}
...
void maju1()
{
	...
	Walking::GetInstance()->Z_MOVE_AMPLITUDE = 37;
...
void maju2()
{
	...
	Walking::GetInstance()->P_OFFSET=-7.5;
	Walking::GetInstance()->HIP_PITCH_OFFSET = 28.0;
    Walking::GetInstance()->Z_MOVE_AMPLITUDE = 37;//foot heigh
		   ...
		   //------------------------------------------------------------------------------------------------morphologi bola
			...
			if (contours1.size() > 0)
			{
		     ...
		     if(StatusCheck::cek_kondisi==2)
		     {
					...
					m_FollowMaxFBStep 	= 35.0; 
					m_FollowMinFBStep 	= 15.0;
					m_FollowMaxRLTurn 	= 20.0; // awal 20
					m_FitFBStep 		= 3.0;
					m_FitMaxRLTurn 		= 13.0;
					m_UnitFBStep 		= 0.33; //1
...
if(DEBUG_PRINT == true)
	...
	if(posX == -1.0 || posY == -1.0)
		{
			...
			if(m_NoBallCount > m_NoBallMaxCount)
			{
				...
				if(Putar==0)
				{
					...
					{
						Walking::GetInstance()->Start();
						Walking::GetInstance()->P_OFFSET=-8.3;
	...
	//	if(pan > m_KickRightAngle && pan < m_KickLeftAngle)
		//{
  				...
  				if(m_KickBallCount >= 0 )
					{
						m_FBStep = 0;
						m_RLTurn = 0;
						//Walking::GetInstance()->HIP_PITCH_OFFSET = 23.0;
						if(DEBUG_PRINT == true)
							fprintf(stderr, "[KICK]");
							berhenti();
							usleep(1000000);
			...
			else
			{
				...
				m_GoalRLTurn = m_FollowMaxRLTurn * pan_percent * 1.8;
				if(DEBUG_PRINT == true)
	...
	if(m_GoalFBStep == 0 && m_GoalRLTurn == 0 && m_FBStep == 0 && m_RLTurn == 0)
		...
		else
		{
			...
			FB_move+=0.33;//kalibrasi dari setting walk tunner
			/*if (FB_move >=0)
			{
				if (FB_move<=3)
				{
