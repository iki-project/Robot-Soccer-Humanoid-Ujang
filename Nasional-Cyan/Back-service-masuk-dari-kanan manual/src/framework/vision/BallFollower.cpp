...
{
	int serial = open("/dev/ttyUSB0", O_RDWR | O_NOCTTY | O_SYNC);
...
if(DEBUG_PRINT == true)
	...
	//	if(pan > m_KickRightAngle && pan < m_KickLeftAngle)
		//{
			...
			if(tilt<=-12.8)//(tilt<= 10  && tilt >= 5)//8.8
			{
  				...
  				if(m_KickBallCount >= 0 )
					{
						...
						if(DEBUG_PRINT == true)
							fprintf(stderr, "[KICK]");
							berhenti();
							usleep(1000000);//8detik
							Tendang();
							usleep(1000000);
							Action::GetInstance()->Start(216); //bola kecil 204
							usleep(5000000);
						//	Tendang();
						//	usleep(1000000);
						//	Action::GetInstance()->Start(204); 
						//	usleep(5000000);
						//	cout<<"Tendangan = "<<nilai_tendang<<endl;
						//	Tendang();
						//	usleep(500000);
						//	Action::GetInstance()->Start(204); 
						//	usleep(3000000);
			...
			else
			{
				...
				m_GoalRLTurn = m_FollowMaxRLTurn * pan_percent * 1;//1.8
				if(DEBUG_PRINT == true)
